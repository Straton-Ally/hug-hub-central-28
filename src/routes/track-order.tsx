...
export const Route = createFileRoute("/track-order")({
  head: () => pageHead("Track an Order", "Sign in to view and track your Spares Automation orders.", "/track-order", true),
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (active) setError("We could not load your orders. Please refresh and try again.");
      } finally {
        ...
        <section className="border-b border-rule bg-charcoal-deep text-white">
          <div className="mx-auto max-w-[1200px] px-4 py-8 md:px-6 md:py-10">
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-white/60 md:text-base">
              Sign in to your account to see live status and courier tracking for every order.
            </p>
          </div>
        </section>
        ...
        <div className="mx-auto max-w-[1200px] px-4 py-10 md:px-6 md:py-14">
          ...
          {loading ? (
            <div className="flex items-center gap-2 border border-rule bg-surface px-4 py-12 font-mono text-[10px] uppercase tracking-[0.25em] text-ink-muted md:px-8 md:py-16">
              <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" /> Loading your orders
            </div>
          ) : !customer ? (
            <SignInRequired
            ...
            <section className="border border-rule bg-surface p-5 md:p-8">
              <h2 className="font-display text-xl font-bold uppercase tracking-tight md:text-2xl">Order history unavailable</h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-ink-muted">
                Order history is not available through this store account yet. Contact the support desk and we will locate your order.
              </p>
            </section>
          ) : orders.length === 0 ? (
            <section className="border border-rule bg-surface p-5 md:p-8">
              <PackageSearch aria-hidden="true" className="mb-4 h-8 w-8 text-accent" />
              <h2 className="font-display text-xl font-bold uppercase tracking-tight md:text-2xl">No orders yet</h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-ink-muted">
                There are no orders linked to your account. Once you place an order, its tracking will appear here.
              </p>
              <Link
                to="/products"
            ...
            <section aria-labelledby="track-orders-title" className="border border-rule bg-surface p-5 md:p-8">
              <div className="flex flex-col gap-2 border-b border-rule pb-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <div className="font-mono text-[10px] uppercase tracking-[0.28em] text-ink-muted">Your orders</div>
                  <h2 id="track-orders-title" className="mt-2 font-display text-xl font-bold uppercase tracking-tight md:text-2xl">Order tracking</h2>
                </div>
